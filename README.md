...
BixChat是一款功能强大的跨平台Agent应用程序，支持Windows和Linux系统。通过ReAct架构，即思考、行动和观察，能够帮助使用研究人员完成复杂的生物信息分析任务。于此同时，我们提供了多种方式便于用户快速集成新的工具，其中包括了MCP服务和自定义工具工具，因此，BixChat是一个通用的架构，能够在根据研究人员的需求进行定制化的配置。

## 软件核心

### biotools

- 我们提供了一个通用的生物信息MCP服务[biotools](./biotools)，用户可以使用本软件或第三方客户端快速接入。

### 虚拟化环境

- 为了防止Agent执行系统级别命令对本地环境造成破坏，我们使用的Docker虚拟化技术，所有执行的结果都将保存在用户指定的目录。与此同时，我们的工具可以执行远程SSH指令，因此用户能在高性能远程服务器上执行分析。

### ReACT

- 我们使用ReACT作为Agent核心架构，于此同时，我们提供了多种执行模式，其中包括`自动模式`，`执行模式`和`规划模式`。这使得我们的软件能以多种方式帮助用户自动完成数据分析任务。
