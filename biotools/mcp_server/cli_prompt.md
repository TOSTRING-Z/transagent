...
- chipseeker: 用于ChIP-seq数据的注释
    Example: mkdir -p output_dir && Rscript -e 'library(ChIPseeker); peakAnno <- annotatePeak("peaks.bed", tssRegion=c(-1000, 1000), TxDb=TxDb.Hsapiens.UCSC.hg38.knownGene); write.csv(peakAnno@annoStat,"output_dir/ChIPseeker_annoStat.csv")'
...
- BETA: Find Target Genes with only binding data: regulatiry potential score
    Example: awk '{print $1"\t"$2"\t"$3}' input_bed > BETA_input_bed_path && BETA minus -p BETA_input_bed_path -g hg38 -n BETA_targets -o output_dir
- fastqc: 用于测序数据的质量控制
